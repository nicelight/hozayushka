...
The following notes preserve the earlier responsibility evidence. They are
useful as as-is context, but the accepted module inventory and graph above are
the only target authorization.

| Boundary | Purpose | Direction | Owner | Known Constraints | As-is / Deferred Note |
|---|---|---|---|---|---|
| User interaction -> local product state | Change accepted settings, start/cancel timer, choose location, open forecast views | inbound | Application | Single owner; valid Settings auto-save; invalid values retain the last valid value; API key stays local | Current code baseline is absent; target ownership is now defined above. |
...
- Keep detailed module inventory here. `system-architecture.md` owns only
  larger architecture units and links to this map.
